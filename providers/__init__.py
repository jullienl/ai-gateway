...
`gpt-5.6-sol`, `claude-sonnet-5`, ...) defaults to `copilot`, so every existing
caller and every shipped agent in agents.py keeps working unchanged.
...
def parse_model_id(model_id: str) -> tuple[str, str]:
    ...
    No `:` present -> `(DEFAULT_PROVIDER, model_id)` unchanged, so bare Copilot
    model ids keep resolving exactly as before this module existed.
    """
